...
while True:
    ...
    while True: 
        ...
        try:
            if reply=='51':
                s1.send([51,{'pin':5,'freq':100},{'pin':4,'freq':100},{},{},{},configed,{'mpu6050':False}])
                break
        except:pass
        s1.close()
    #初始化数据
    ...
    while True:
        ...
        if button[9]:
            axis1[1]=-1
            zeroNum[1]=0
        reply={'duty0':toDuty(config[0],axis1[1]),'duty1':toDuty(config[1],axis1[3]),'config':None,'mes':None}#本次reply
        if button[5]:
            reply['mes']='break'
